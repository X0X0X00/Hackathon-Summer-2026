...
V2A_EI_OOF = 0.7690
MODEL_V1_OOF = 0.7598
CANDIDATE_REL = "work/outputs/submissions/model_v2_candidate.csv"
...
def main() -> int:
    ...
    write_oof(
        ROOT / "work/outputs/oof/MODEL-V2_oof.csv",
        train_ids,
        y_true,
        selected["pred"],
        fold_ids,
    ...
    write_proba(
        ROOT / "work/outputs/probabilities/MODEL-V2_oof_probabilities.csv.gz",
        train_ids,
        blend_probas[selected_id],
        class_names,
    )
    write_proba(
        ROOT / "work/outputs/probabilities/MODEL-V2_test_probabilities.csv.gz",
        test_ids,
        test_mixed,
        class_names,
    )
    write_confusion(
        ROOT / "work/outputs/metrics/MODEL-V2_confusion.csv",
        y_true,
        selected["pred"],
        class_names,
    )
    ...
    write_json(
        ROOT / "work/outputs/metrics/V2-C-scoreboard.json",
        {
            "expert_files": {
                "A_oof": EXPERT_A_OOF,
    ...
    write_json(
        ROOT / "work/outputs/metrics/V2-C-complementarity.json",
        {
            "headroom_only": True,
    ...
    write_json(
        ROOT / "work/outputs/metrics/model_v2_selection.json",
        {
            "model_name": "MODEL V2",
            "selected_id": selected_id,
            "architecture": (
