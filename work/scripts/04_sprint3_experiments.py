...
def load_oof_frame(run_id: str) -> pd.DataFrame:
    return pd.read_csv(
        ROOT / "work" / "outputs" / "oof" / "{}_oof.csv".format(run_id),
...
def record_run(payload: dict, overwrite: bool) -> None:
    ...
    row = {
        ...
        "metrics_path": "work/outputs/metrics/{}_metrics.json".format(payload["run_id"]),
...
def run_yw005(cell_ids, y, folds, class_names, yw004_pred) -> dict:
    ...
    payload = {
        ...
        "n_cells": int(len(y)),
        "oof_path": "work/outputs/oof/YW-005_oof.csv",
        "proba_path": "work/outputs/probabilities/YW-005_oof_probabilities.csv.gz",
        "conclusion": (
    ...
    write_oof(ROOT / "work/outputs/oof/YW-005_oof.csv", cell_ids, y, pred, folds)
    write_proba(ROOT / "work/outputs/probabilities/YW-005_oof_probabilities.csv.gz", cell_ids, p_cf, class_names)
    write_json(ROOT / "work/outputs/metrics/YW-005_metrics.json", payload)
    return payload
...
def run_yw006(data, cell_ids, y, folds, class_names, sigs, yw004_pred) -> dict:
    ...
    payload = {
        ...
        "oof_path": "work/outputs/oof/YW-006_hard_bucket_oof.csv",
        "proba_path": "work/outputs/probabilities/YW-006_hard_bucket_probabilities.csv.gz",
        "conclusion": (
    ...
    write_oof(
        ROOT / "work/outputs/oof/YW-006_hard_bucket_oof.csv",
        ids_b,
        y_b,
        pred_b,
        fold_b,
    ...
    write_proba(
        ROOT / "work/outputs/probabilities/YW-006_hard_bucket_probabilities.csv.gz",
        ids_b,
        proba_global[bucket_mask],
        class_names,
    )
    write_json(ROOT / "work/outputs/metrics/YW-006_metrics.json", payload)
...
def run_yw007(cell_ids, y, folds, class_names, sigs, yw004_oof, yw004_proba, yw006_payload) -> dict:
    ...
    payload = {
        ...
        "n_cells": 5000,
        "oof_path": "work/outputs/oof/YW-007_oof.csv",
        "proba_path": "work/outputs/probabilities/YW-007_oof_probabilities.csv.gz",
        "conclusion": (
    ...
    write_oof(ROOT / "work/outputs/oof/YW-007_oof.csv", cell_ids, y, pred, folds)
    write_proba(ROOT / "work/outputs/probabilities/YW-007_oof_probabilities.csv.gz", cell_ids, proba, class_names)
    write_json(ROOT / "work/outputs/metrics/YW-007_metrics.json", payload)
    return payload, pred
...
def main() -> int:
    ...
    folds_df = load_folds(ROOT / "work" / "experiments" / "folds.csv")
    ...
    for run_id in args.run:
        ...
        if run_id == "YW-005":
            ...
            if yw006_payload is None:
                ...
                m006 = json.loads((ROOT / "work/outputs/metrics/YW-006_metrics.json").read_text())
                hb = pd.read_csv(
                    ROOT / "work/outputs/oof/YW-006_hard_bucket_oof.csv",
                    dtype={"Cell_ID": str, "true_label": str, "predicted_label": str, "fold": int},
                )
