...
| Submission candidate | `work/experiments/evidence/submissions/model_v2_candidate.csv` |
...
The candidate has exactly 5000 test predictions in `meta_test.csv` order, uses the official columns `Cell_ID,MERFISH_cell_type_annotation.y`, and is produced only from the V2-B-REFONLY Segment-masked test probabilities (historically `outputs/probabilities/V2-B-REFONLY_test_probabilities_seg.csv.gz`; regenerable under `work/outputs/`).
...
The raw `.h5ad` lives under `work/external/`, which is **gitignored**. It is not distributed in Git. Reproduction requires placing that file locally and verifying the MD5 above. This repository records Zenodo record **18039571** and the expected filename/MD5; it does not record a download URL.
...
All MODEL V2 numbers use the team-compatible 5-fold file `work/experiments/team_folds_5_seed42.csv`:
...
- Frozen team 5-fold file; personal 3-fold `work/experiments/folds.csv` untouched.
...
work/.venv/bin/python work/scripts/07_bridge_yw004_5f.py

# V2-A competition-only spatial LightGBM
work/.venv/bin/python work/scripts/08_v2a_spatial_lgbm.py
...
work/.venv/bin/python work/scripts/11_v2c_select.py

work/.venv/bin/python -m pytest -q work/tests
work/.venv/bin/python work/scripts/90_validate_submission.py work/experiments/evidence/submissions/model_v2_candidate.csv
...
| Kind | Path |
|---|---|
| V2-B training | `work/scripts/09_v2b_refonly.py`, `work/src/merfish60/reference.py`, `work/src/merfish60/ext_universe.py` |
| V2-C blend (no fit) | `work/scripts/11_v2c_select.py`, `work/src/merfish60/v2c_blend.py` |
| Folds | `work/experiments/team_folds_5_seed42.csv` |
| Registry | `work/experiments/registry_v2.csv` |
| V2-B metrics / audits | `work/experiments/evidence/metrics/V2-B-REFONLY_*.json`, `work/reports/V2-B-REFONLY_methodology.json` |
| V2-C scoreboard | `work/experiments/evidence/metrics/V2-C-scoreboard.json`, `work/experiments/evidence/metrics/V2-C-complementarity.json` |
| Candidate | `work/experiments/evidence/submissions/model_v2_candidate.csv` |
| Release metrics | `work/experiments/evidence/metrics/model_v2_metrics.json` |
