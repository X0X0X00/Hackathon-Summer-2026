...
def test_validation_and_test_probability_contracts():
    _require_outputs()
    if not (OUT / "v3_e06m_m2_validation_probabilities.csv.gz").is_file():
        pytest.skip("regenerable E06M probability dumps not retained")
...
def test_report_contains_required_decision_and_oracle_warning():
    _require_outputs()
    text = (ROOT / "work/reports/v3/v3_e06m_source_balanced_multireference.md").read_text()
