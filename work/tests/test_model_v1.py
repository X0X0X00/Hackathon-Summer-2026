...
MODEL_V1_SOURCES = [
    ROOT / "work" / "src" / "merfish60" / "model_v1.py",
    ROOT / "work" / "scripts" / "06_model_v1.py",
]
...
def _require(rel: str) -> Path:
    path = ROOT / rel
    if not path.is_file():
        pytest.skip("missing {}; regenerate with work/scripts/06_model_v1.py if needed".format(rel))
    return path
...
def test_no_metadata_features_in_specialists(fitted):
    ...
    src = (ROOT / "work" / "src" / "merfish60" / "model_v1.py").read_text()
...
def test_model_v1_csv_row_count_and_headers():
    path = _require("work/experiments/evidence/submissions/model_v1.csv")
...
def test_model_v1_probability_order_and_row_sums(class_names):
    order = json.loads(_require("work/experiments/evidence/metrics/model_v1_class_order.json").read_text())
    assert order["class_order"] == class_names
    assert order["n_classes"] == 60
    proba = pd.read_csv(
        _require("work/outputs/probabilities/model_v1_test_probabilities.csv.gz"),
        dtype={"Cell_ID": str},
        compression="gzip",
    )
    sub = pd.read_csv(
        _require("work/experiments/evidence/submissions/model_v1.csv"),
        dtype={"Cell_ID": str},
    )
...
def test_metrics_record_frozen_selection_not_test_accuracy():
    metrics = json.loads(_require("work/experiments/evidence/metrics/model_v1_metrics.json").read_text())
    ...
    summary = pd.read_csv(_require("work/experiments/evidence/metrics/model_v1_signature_summary.csv"))
