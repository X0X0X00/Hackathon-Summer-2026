...
V2C_SOURCES = [
    ROOT / "work" / "src" / "merfish60" / "v2c_blend.py",
    ROOT / "work" / "scripts" / "11_v2c_select.py",
]
MODEL_V1_PATHS = [
...
def test_abc_oof_alignment_and_row_sums(data, class_names):
    for rel in (EXPERT_A_OOF, EXPERT_B_OOF, EXPERT_C_OOF):
        if not (ROOT / rel).is_file():
            pytest.skip("generated expert probabilities not retained: {}".format(rel))
    frames = {
    ...
    for name in ("A", "B", "C"):
        oof = pd.read_csv(
            ROOT / "work/outputs/oof/{}_oof.csv".format(
                {
                    "A": "BRIDGE-YW004-5F",
                    "B": "V2-A-SPATIAL-LGBM",
                    "C": "V2-B-REFONLY",
...
def test_model_v2_candidate_contract_if_present():
    path = ROOT / "work" / "experiments" / "evidence" / "submissions" / "model_v2_candidate.csv"
    if not path.is_file():
...
def test_selected_test_proba_row_sums_if_present(class_names):
    path = ROOT / "work" / "outputs" / "probabilities" / "MODEL-V2_test_probabilities.csv.gz"
    if not path.is_file():
