...
- 最终提交模型：`{{SELECTED_MODEL}}`
- 选择规则：{{SELECTION_REASON}}
- OOF Accuracy：`{{FINAL_ACC}}`
- OOF Macro-F1：`{{FINAL_MACRO}}`
- OOF Weighted-F1：`{{FINAL_WEIGHTED}}`
- OOF Log loss：`{{FINAL_LOGLOSS}}`
- 训练设备：`{{DEVICE}}`
- 测试集启用 strict H：`{{ELIGIBLE_TEST}} / 5000` 个细胞（`{{ELIGIBLE_TEST_FRAC}}`）
...
|---|---:|---:|---:|---:|
| Prior-H Anchor | {{ANCHOR_ACC}} | {{ANCHOR_MACRO}} | {{ANCHOR_WEIGHTED}} | {{ANCHOR_LOGLOSS}} |
| 本次训练 graph stacker | {{STACKER_ACC}} | {{STACKER_MACRO}} | {{STACKER_WEIGHTED}} | {{STACKER_LOGLOSS}} |
| 最终自动选择 | {{FINAL_ACC}} | {{FINAL_MACRO}} | {{FINAL_WEIGHTED}} | {{FINAL_LOGLOSS}} |
...
| V3 Prior-H graph stacker | `code/train_final_prior_h_graph_stacker.py` | 用 Prior-H 作锚点，联合 V2、prior、gene-token、strict H，并继续施加类别图正则 | Accuracy {{STACKER_ACC}}，Macro-F1 {{STACKER_MACRO}}，Log loss {{STACKER_LOGLOSS}} |
